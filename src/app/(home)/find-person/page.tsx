...
<<<<<<< HEAD
      showAlert({
        ...
        description: currentLanguage === 'ar' ? 'معرف الشخص المفقود غير متوفر.' : 'Lost person ID is missing.'
      });
=======
...
<<<<<<< HEAD
      showAlert({
        type: 'error',
        title: currentLanguage === 'ar' ? 'خطأ' : 'Error',
        description: currentLanguage === 'ar' ? 'معرف المساعد غير متوفر.' : 'Helper ID is missing.'
      });
=======
...
<<<<<<< HEAD
        ...
        // Show success alert after a brief delay to let the navigation interface render first
        setTimeout(() => {
          showAlert({
            type: 'success',
            title: currentLanguage === 'ar' ? 'نجح بدء الجلسة!' : 'Session started successfully!',
            description: currentLanguage === 'ar' ? 'تم الاتصال بالشخص المفقود' : 'Connected to lost person',
            autoClose: 1500
          });
        }, 100);
      } else {
        showAlert({
          type: 'error',
          title: currentLanguage === 'ar' ? 'خطأ' : 'Error',
          description: currentLanguage === 'ar' ? 'فشل في إنشاء معرف الجلسة' : 'Failed to create session ID'
        });
      }
    } catch {
      showAlert({
        type: 'error',
        title: currentLanguage === 'ar' ? 'خطأ' : 'Error',
        description: currentLanguage === 'ar' ? 'فشل في بدء الجلسة. يرجى المحاولة.' : 'Failed to start session. Please try again.'
      });
    }
  };

  // Show search form or navigation interface
  return (
    <div className="min-h-screen bg-gradient-to-br from-green-50 to-emerald-100">
      <div className="w-full max-w-4xl mx-auto p-4">
        {/* Show search form if no active session */}
        {!activeSessionId && (
...
<<<<<<< HEAD
                      showAlert({
                        type: 'success',
                        title: currentLanguage === 'ar' ? 'تم إنهاء الجلسة بنجاح' : 'Session ended successfully',
                        autoClose: 3000
                      });
=======
                      alert(
...
<<<<<<< HEAD
                    showAlert({
                      type: 'error',
                      title: currentLanguage === 'ar' ? 'خطأ في الاتصال' : 'Connection Error',
                      description: currentLanguage === 'ar'
