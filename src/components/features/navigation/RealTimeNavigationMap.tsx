...
import { MapPin, Navigation, Route, AlertCircle } from "lucide-react";
...
export function RealTimeNavigationMap({ 
      ...
      if ('permissions' in navigator) {
        navigator.permissions.query({ name: 'geolocation' }).then((permissionStatus) => {
          ...
          if (permissionStatus.state === 'denied') {
            setLocationError(errorT.location.permissionDeniedMessage);
            return;
          }
        ...
        (error) => {
          ...
          switch (error.code) {
            case error.PERMISSION_DENIED:
              errorMessage = errorT.location.permissionDenied;
              break;
            case error.POSITION_UNAVAILABLE:
              errorMessage = errorT.location.unavailableMessage;
              break;
            case error.TIMEOUT:
              errorMessage = errorT.location.timeoutMessage;
              break;
            default:
              errorMessage = errorT.location.trackingFailed;
          }
          ...
          searcherMarkerRef.current = new mapboxgl.Marker({ element: searcherEl })
            ...
            .setPopup(new mapboxgl.Popup().setHTML(`
              <div style="text-align: center;">
                <strong>${navT.realTimeNav.searcher}</strong><br>
      ...
      if (lat && lng) {
        ...
        if (lostPersonMarkerRef.current) {
          ...
          lostPersonMarkerRef.current = new mapboxgl.Marker({ element: lostEl })
            ...
            .setPopup(new mapboxgl.Popup().setHTML(`
              <div style="text-align: center;">
                <strong>${navT.realTimeNav.lostPerson}</strong><br>
                <small>${lat.toFixed(6)}, ${lng.toFixed(6)}</small>
              </div>
            `))
            .addTo(map);
    ...
    try {
      ...
      if (distance < 0.0001) { // Very close coordinates (less than ~10 meters)
        setNavigationRoute({
          ...
          directions: [navT.realTimeNav.foundDescription],
  ...
  const formatTime = (time: { minutes: number; seconds: number }): string => {
    if (time.minutes > 0) {
      return `${time.minutes} ${navT.time.minuteShort} ${time.seconds} ${navT.time.secondShort}`;
    } else {
      return `${time.seconds} ${navT.time.seconds}`;
    }
  };
  ...
  return (
    <div className="space-y-4">
      ...
      <Card className="p-3 sm:p-4">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className={`text-lg sm:text-xl font-semibold ${dir.textAlign}`}>
                {navT.realTimeNav.title}
              </h2>
              <p className={`text-xs sm:text-sm text-gray-600 ${dir.textAlign}`}>
                {userType === 'searcher' 
                  ? navT.realTimeNav.searcherDescription
                  : navT.realTimeNav.lostPersonDescription
                }
              </p>
            </div>
          </div>
          <div className="flex items-center gap-2 flex-wrap">
            <Badge variant={sessionConnected && profileConnected ? 'default' : 'destructive'} className="animate-pulse text-xs">
            ...
            {userType === 'searcher' && (
              <Button variant="outline" size="sm" onClick={onClose} className="hover:bg-red-50 hover:border-red-300 text-xs sm:text-sm">
                {navT.realTimeNav.endSession}
              </Button>
            )}
          </div>
        </div>
        ...
        <Card className="p-4 bg-yellow-50 border-yellow-200">
          <div className="flex items-center gap-2">
            ...
            <span className="text-yellow-800">
              {navT.realTimeNav.reconnectingMessage}
            </span>
          </div>
        </Card>
      )}
      ...
      <Card className="p-2">
        <div 
          ref={mapRef} 
          className="w-full h-64 sm:h-80 md:h-96 lg:h-[28rem] rounded-lg overflow-hidden border-2 border-blue-200"
          ...
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 text-sm">
            <div className="flex items-center gap-2 sm:gap-4 flex-wrap">
              <div className="flex items-center gap-1">
                ...
                <span className="text-blue-800 text-xs sm:text-sm">{navT.realTimeNav.searcher}</span>
              </div>
              <div className="flex items-center gap-1">
                <div className="w-3 h-3 bg-red-600 rounded-full border border-red-800"></div>
                <span className="text-red-800 text-xs sm:text-sm">{navT.realTimeNav.lostPerson}</span>
              </div>
            </div>
            <div className="text-gray-600 text-xs sm:text-sm">
              {navT.realTimeNav.liveUpdates} • 
              {bothLocationsAvailable 
                ? navT.status.connected
                : navT.realTimeNav.waitingForBothLocations
              }
            </div>
          </div>
        </div>
      ...
      <Card className="p-4">
          <div className="flex items-center gap-2 mb-4">
            ...
            <h3 className={`font-semibold ${dir.textAlign}`}>
              {navT.realTimeNav.navigationInfo}
            </h3>
            {isCalculatingRoute && (
              ...
              {navigationRoute ? (
                <>
                  ...
                  {navigationRoute.distance === 0 ? (
                    <div className="bg-green-50 rounded-lg p-4 text-center border-2 border-green-200">
                      ...
                      <h4 className="font-bold text-green-900 mb-2">
                        {navT.realTimeNav.foundMessage}
                      </h4>
                      <p className="text-green-700">
                        {navT.realTimeNav.foundDescription}
                      </p>
                    </div>
                  ) : (
                    <div className="bg-purple-50 rounded-lg p-3">
                      ...
                      <div className="flex items-center justify-between">
                        <span className="font-medium text-purple-900">
                          {navT.realTimeNav.estimatedTime}
                        </span>
                        <span className="text-purple-700 font-mono">
                  ...
                  {navigationRoute.directions && navigationRoute.directions.length > 0 && (
                    <div className="bg-muted rounded-lg p-3 max-h-32 overflow-y-auto theme-transition">
                      <h4 className={`font-medium mb-2 text-foreground ${dir.textAlign}`}>
                        {navT.realTimeNav.directions}
                      </h4>
                      <ol className={`text-sm space-y-1 ${dir.textAlign}`}>
                        {navigationRoute.directions.map((direction, index) => (
                          <li key={index} className="flex items-start gap-2">
                            <span className="text-xs bg-muted-foreground/20 text-muted-foreground rounded-full w-5 h-5 flex items-center justify-center flex-shrink-0 mt-0.5">
                              {index + 1}
                            </span>
                            <span className="text-foreground">
                              {typeof direction === 'string' 
                                ? direction 
                                : (direction as { instruction?: string; distance?: number }).instruction || navT.errors.unknownError
                              }
                            </span>
                          </li>
                        ))}
                ...
                <div className="text-center py-4">
                  <p className="text-gray-500">
                    {navT.realTimeNav.calculatingRoute}
                  </p>
                  <Button 
                    size="sm" 
                    variant="outline" 
