...
export function LostPersonNavigationInterface({ 
  ...
  const getStatusMessage = (): { text: string; color: string; icon: React.JSX.Element } => {
    if (!sessionConnected) {
      return {
        text: currentLanguage === 'ar' ? 'لا يوجد اتصال' : 'No Connection',
        color: 'text-gray-500',
    ...
    if (searcherConfirmed) {
      return {
        text: currentLanguage === 'ar' ? 'تم العثور عليك!' : 'You have been found!',
    ...
    if (distance && distance < 20) {
      return {
        text: currentLanguage === 'ar' ? 'الباحث قريب جداً!' : 'Searcher is very close!',
        color: 'text-green-600',
    ...
    if (distance && distance < 100) {
      return {
        text: currentLanguage === 'ar' ? 'الباحث يقترب' : 'Searcher approaching',
        color: 'text-blue-600',
    ...
    return {
      text: currentLanguage === 'ar' ? 'باحث في الطريق إليك' : 'Searcher heading to you',
      color: 'text-blue-600',
  ...
  const translations = {
    title: currentLanguage === 'ar' ? 'الباحث في الطريق إليك' : 'Searcher Coming to You',
    subtitle: currentLanguage === 'ar' ? 'ابق في مكانك وانتظر وصول الباحث' : 'Stay in your location and wait for the searcher',
    searcherInfo: currentLanguage === 'ar' ? 'معلومات الباحث' : 'Searcher Information',
    distance: currentLanguage === 'ar' ? 'المسافة' : 'Distance',
    eta: currentLanguage === 'ar' ? 'الوقت المتوقع للوصول' : 'Estimated Arrival',
    instructions: currentLanguage === 'ar' ? 'تعليمات' : 'Instructions',
    stayInPlace: currentLanguage === 'ar' ? 'ابق في مكانك الحالي' : 'Stay in your current location',
    lookOut: currentLanguage === 'ar' ? 'انتبه للباحث الذي يقترب منك' : 'Look out for the approaching searcher',
    confirmButton: currentLanguage === 'ar' ? 'يمكنني رؤية الباحث!' : 'I can see the searcher!',
    callButton: currentLanguage === 'ar' ? 'اتصال' : 'Call',
    mapButton: currentLanguage === 'ar' ? 'عرض الخريطة' : 'View Map',
    backButton: currentLanguage === 'ar' ? 'عودة' : 'Back',
    liveTracking: currentLanguage === 'ar' ? 'التتبع المباشر' : 'Live Tracking',
    connectionStatus: currentLanguage === 'ar' ? 'حالة الاتصال' : 'Connection Status'
  };
    ...
    return (
      <ErrorBoundary
        ...
        fallbackMessage={currentLanguage === 'ar' 
          ? 'حدث خطأ في عرض الخريطة. يرجى إعادة المحاولة.' 
          : 'An error occurred displaying the map. Please try again.'
        }
      >
    ...
    <div className="space-y-4 max-w-4xl mx-auto p-4">
            ...
            <div className="flex gap-4">
              <div className="flex items-center gap-2">
                ...
                <div className="text-center">
                  <p className="text-xs text-gray-500">{translations.distance}</p>
              ...
              <div className="flex items-center gap-2">
                ...
                <div className="text-center">
                  <p className="text-xs text-gray-500">{translations.eta}</p>
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-gray-500">
                {currentLanguage === 'ar' ? 'متطوع للبحث' : 'Search Volunteer'}
              </p>
            </div>
          </div>
        </Card>
      ...
      <Card className="p-4 bg-yellow-50 border-yellow-200">
        ...
        <ul className={`text-sm text-yellow-700 space-y-1 ${dir.textAlign}`}>
          <li className="flex items-center gap-2">
            ...
            {translations.stayInPlace}
          </li>
          <li className="flex items-center gap-2">
            <div className="w-2 h-2 bg-yellow-600 rounded-full"></div>
            {translations.lookOut}
          </li>
          {searcherName && (
            <li className="flex items-center gap-2">
              <div className="w-2 h-2 bg-yellow-600 rounded-full"></div>
              {currentLanguage === 'ar' 
                ? `اسم الباحث: ${searcherName}`
      ...
      <Card className="p-3">
        <div className="flex items-center justify-between">
          <span className="text-sm text-gray-600">{translations.connectionStatus}:</span>
